...
for $$(x/R)$$ in the range $$(-2, 2)$$ when $$y/R=1$$ instead. Even though the question says *plot the magnitude of the electric field* (which is technically $$ \lvert \vec{E} \rvert $$), this cannot be done numerically, so you'll have to plot $$4 \pi \epsilon_0 R^2 \abs{\vec{E}}/q$$ the instead.
